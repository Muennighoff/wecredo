...
def write_to_file(writer, input_ids):
    """
...
def write_files(input_ids_list_array, files_per, output_dir, out_name, start_no, write_remainder=False, process_no=None):
    ...
    if len(input_ids_list_array[-1]) != files_per and not write_remainder: # pop the last file if it's length != files per
        ...
        input_ids_remainder = None # assuming files = remainder from an old chunk here
        files_per = len(input_ids_list_array[-1])

    for input_ids_list in input_ids_list_array:
        ...
        start_no += 1
    return start_no, input_ids_remainder
...
def create_tfrecords(params, write_remainder=True, write_every_n_files=1, resume_from_checkpoint=True, display_pbar=True):
    ...
    enc = BertTokenizerFast.from_pretrained("mymusise/gpt2-medium-chinese")
    ...
    for f in files:
        ...
        if f in resume_files_processed:
            continue
        for input_ids_list in archive_to_tokens(f, enc, args): # input_ids_list is a whole file chunked in lists of seq_len
            files_processed += 1
            ...
            if len(input_ids_list_array) >= args.files_per * write_every_n_files: # write every n files
    ...
    if len(input_ids_list_array) >= args.files_per: # also write at end
        _tfrecord_count, input_ids_remainder = write_files(input_ids_list_array, files_per=args.files_per, output_dir=args.output_dir, out_name=args.name, start_no=tfrecord_count, process_no=process_no)
    ...
    if write_remainder:
        ...
        write_files(input_ids_list_array, files_per=args.files_per, output_dir=args.output_dir, out_name=args.name, start_no=tfrecord_count, write_remainder=True)
...
if __name__ == "__main__":
    ...
    args.seq_len += 1 # we shift the data by 1 to the right for targets, so increment the chunk size here
